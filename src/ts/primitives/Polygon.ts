import { IPoint, IPolygon, ISegment } from '../../types/types';
import { average, getIntersection, getRandomColor } from '../utils';
...
export class Polygon {
  ...
  static multiBreak(polys: IPolygon[]) {
    for (let i = 0; i < polys.length; i++) {
  ...
  static break(poly1: IPolygon, poly2: IPolygon) {
  ...
  distanceToPoly(poly: IPolygon) {
    return Math.min(...this.points.map((p) => poly.distanceToPoint(p)));
  }

  intersectsPoly(poly: IPolygon) {
